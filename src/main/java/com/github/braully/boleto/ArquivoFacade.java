    ...
    public void parse(List<String> linhas) {
        ...
        for (String linha : linhas) {
            ...
            for (RegistroArquivo reg : registrosLayout) {
                ...
                if (idType != null && id != null && id.getValue() != null && !id.getValue().isBlank() && id.getValue().equals(idType.getValue())) {
                    regLido = reg.clone();
                    Integer fixedLength = reg.getFixedLength();
                    regLido.read(linha);
                }
            }
            if (regLido != null) {
